<div class="col-12">
    <div class="card card-primary card-tabs">
        <div class="card-header p-0 pt-1">
            <ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist">
                <!-- //* 2023/2024 -->
                <li class="nav-item">
                    <a class="nav-link active" id="custom-tabs-one-2324-tab" data-toggle="pill" href="#custom-tabs-one-2324" role="tab" aria-controls="custom-tabs-one-group" aria-selected="true">23/24</a>
                </li>
                <!-- //* 2022/2023 -->
                <li class="nav-item">
                    <a class="nav-link" id="custom-tabs-one-2223-tab" data-toggle="pill" href="#custom-tabs-one-2223" role="tab" aria-controls="custom-tabs-one-2223" aria-selected="false">22/23</a>
                </li>
            </ul>
        </div>
        <div class="card-body">
            <div class="tab-content" id="custom-tabs-one-tabContent">
                <!-- //* 2023/2024 -->
                <div class="tab-pane fade show active" id="custom-tabs-one-2324" role="tabpanel" aria-labelledby="custom-tabs-one-2324-tab">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="card card-primary">
                                <div class="card-body">
                                    <table id="2324" class="table table-bordered">
                                        <thead>
                                            <tr class="text-center">
                                                <th style="border: none;" width="1px"></th>
                                                <th>No</th>
                                                <th width="500px">Tim</th>
                                                <th>M</th>
                                                <th>M</th>
                                                <th>S</th>
                                                <th>K</th>
                                                <th>GM</th>
                                                <th>GK</th>
                                                <th>SG</th>
                                                <th>Poin</th>
                                            </tr>
                                        </thead>
                                    </table>
                                </div>
                                <!-- /.card-body -->
                            </div>
                            <!-- /.card -->
                        </div>
                    </div>
                </div>
                <!-- //* 2022/2023 -->
                <div class="tab-pane fade" id="custom-tabs-one-2223" role="tabpanel" aria-labelledby="custom-tabs-one-2223-tab">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="card card-primary">
                                <div class="card-body">
                                    <table id="2223" class="table table-bordered">
                                        <thead>
                                            <tr class="text-center">
                                                <th style="border: none;" width="1px"></th>
                                                <th>No</th>
                                                <th width="500px">Tim</th>
                                                <th>M</th>
                                                <th>M</th>
                                                <th>S</th>
                                                <th>K</th>
                                                <th>GM</th>
                                                <th>GK</th>
                                                <th>SG</th>
                                                <th>Poin</th>
                                            </tr>
                                        </thead>
                                    </table>
                                </div>
                                <!-- /.card-body -->
                            </div>
                            <!-- /.card -->
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>

<script>
    function DataLigue1() {
        //* 23/24
        $.ajax({
            url: 'https://api-football-standings.azharimm.dev/leagues/ksa.1/standings',
            method: 'GET',
            dataType: 'JSON',
            data: {
                'season': '2023',
                'sort': 'asc'
            },
            success: function(result) {
                if (result) {
                    let standings = result.data.standings;
                    let tbodyContent = '';

                    $.each(standings, function(i, data) {
                        let rowClass = (i == 0 || i == 1) ? 'bg-blue color-palette' : (i == 15 || i == 16 || i == 17 ? 'bg-red color-palette' : '');

                        tbodyContent += `
                            <tr class="text-center">
                                <td style="border: none;" class="${rowClass}"></td>
                                <td>${i + 1}</td>
                                <td>${data.team.displayName}</td>
                                <td>${data.stats[0].displayValue}</td>
                                <td>${data.stats[7].displayValue}</td>
                                <td>${data.stats[6].displayValue}</td>
                                <td>${data.stats[1].displayValue}</td>
                                <td>${data.stats[5].displayValue}</td>
                                <td>${data.stats[4].displayValue}</td>
                                <td>${data.stats[2].displayValue}</td>
                                <td>${data.stats[3].displayValue}</td>
                            </tr>
                        `;
                    });
                    $('#custom-tabs-one-2324 tbody').html(tbodyContent);
                };
            }
        });
        //* 22/23
        $.ajax({
            url: 'https://api-football-standings.azharimm.dev/leagues/ksa.1/standings',
            method: 'GET',
            dataType: 'JSON',
            data: {
                'season': '2022',
                'sort': 'asc'
            },
            success: function(result) {
                if (result) {
                    let standings = result.data.standings;
                    let tbodyContent = '';

                    $.each(standings, function(i, data) {
                        let rowClass = (i == 0) ? 'bg-blue color-palette' : (i == 1) ? 'bg-info color-palette' : (i == 14 || i == 15 ? 'bg-red color-palette' : '');

                        tbodyContent += `
                            <tr class="text-center">
                                <td style="border: none;" class="${rowClass}"></td>
                                <td>${i + 1}</td>
                                <td>${data.team.displayName}</td>
                                <td>${data.stats[0].displayValue}</td>
                                <td>${data.stats[7].displayValue}</td>
                                <td>${data.stats[6].displayValue}</td>
                                <td>${data.stats[1].displayValue}</td>
                                <td>${data.stats[5].displayValue}</td>
                                <td>${data.stats[4].displayValue}</td>
                                <td>${data.stats[2].displayValue}</td>
                                <td>${data.stats[3].displayValue}</td>
                            </tr>
                        `;
                    });
                    $('#custom-tabs-one-2223 tbody').html(tbodyContent);
                };
            }
        });
        //* 21/22
        $.ajax({
            url: 'https://api-football-standings.azharimm.dev/leagues/idn.1/standings',
            method: 'GET',
            dataType: 'JSON',
            data: {
                'season': '2021',
                'sort': 'asc'
            },
            success: function(result) {
                if (result) {
                    let standings = result.data.standings;
                    let tbodyContent = '';

                    $.each(standings, function(i, data) {
                        let rowClass = (i == 0) ? 'bg-blue color-palette' : (i == 15 || i == 16 || i == 17 ? 'bg-red color-palette' : '');

                        tbodyContent += `
                            <tr class="text-center">
                                <td style="border: none;" class="${rowClass}"></td>
                                <td>${i + 1}</td>
                                <td>${data.team.displayName}</td>
                                <td>${data.stats[0].displayValue}</td>
                                <td>${data.stats[7].displayValue}</td>
                                <td>${data.stats[6].displayValue}</td>
                                <td>${data.stats[1].displayValue}</td>
                                <td>${data.stats[5].displayValue}</td>
                                <td>${data.stats[4].displayValue}</td>
                                <td>${data.stats[2].displayValue}</td>
                                <td>${data.stats[3].displayValue}</td>
                            </tr>
                        `;
                    });
                    $('#custom-tabs-one-2122 tbody').html(tbodyContent);
                };
            }
        });
        //* 20/21
        $.ajax({
            url: 'https://api-football-standings.azharimm.dev/leagues/idn.1/standings',
            method: 'GET',
            dataType: 'JSON',
            data: {
                'season': '2020',
                'sort': 'asc'
            },
            success: function(result) {
                if (result) {
                    let standings = result.data.standings;
                    let tbodyContent = '';

                    $.each(standings, function(i, data) {
                        let rowClass = (i == 0) ? 'bg-blue color-palette' : (i == 1) ? 'bg-orange color-palette' : (i == 15 || i == 16 || i == 17) ? 'bg-red color-palette' : '';

                        tbodyContent += `
                            <tr class="text-center">
                                <td style="border: none;" class="${rowClass}"></td>
                                <td>${i + 1}</td>
                                <td>${data.team.displayName}</td>
                                <td>${data.stats[0].displayValue}</td>
                                <td>${data.stats[7].displayValue}</td>
                                <td>${data.stats[6].displayValue}</td>
                                <td>${data.stats[1].displayValue}</td>
                                <td>${data.stats[5].displayValue}</td>
                                <td>${data.stats[4].displayValue}</td>
                                <td>${data.stats[2].displayValue}</td>
                                <td>${data.stats[3].displayValue}</td>
                            </tr>
                        `;
                    });
                    $('#custom-tabs-one-2021 tbody').html(tbodyContent);
                };
            }
        });
        //* 19/20
        $.ajax({
            url: 'https://api-football-standings.azharimm.dev/leagues/idn.1/standings',
            method: 'GET',
            dataType: 'JSON',
            data: {
                'season': '2019',
                'sort': 'asc'
            },
            success: function(result) {
                if (result) {
                    let standings = result.data.standings;
                    let tbodyContent = '';

                    $.each(standings, function(i, data) {
                        let rowClass = (i == 0) ? 'bg-blue color-palette' : (i == 1) ? 'bg-orange color-palette' : (i == 15 || i == 16 || i == 17) ? 'bg-red color-palette' : '';

                        tbodyContent += `
                            <tr class="text-center">
                                <td style="border: none;" class="${rowClass}"></td>
                                <td>${i + 1}</td>
                                <td>${data.team.displayName}</td>
                                <td>${data.stats[0].displayValue}</td>
                                <td>${data.stats[7].displayValue}</td>
                                <td>${data.stats[6].displayValue}</td>
                                <td>${data.stats[1].displayValue}</td>
                                <td>${data.stats[5].displayValue}</td>
                                <td>${data.stats[4].displayValue}</td>
                                <td>${data.stats[2].displayValue}</td>
                                <td>${data.stats[3].displayValue}</td>
                            </tr>
                        `;
                    });
                    $('#custom-tabs-one-1920 tbody').html(tbodyContent);
                };
            }
        });
    };

    $(document).ready(function() {
        DataLigue1();
    });
</script>

<script>
    $("#2324").DataTable({
        "responsive": true,
        "lengthChange": true,
        "autoWidth": false,
        "paging": false,
        "searching": true,
        "ordering": false,
        "info": false,
    });
    $("#2223").DataTable({
        "responsive": true,
        "lengthChange": true,
        "autoWidth": false,
        "paging": false,
        "searching": true,
        "ordering": false,
        "info": false,
    });
    $("#2122").DataTable({
        "responsive": true,
        "lengthChange": true,
        "autoWidth": false,
        "paging": false,
        "searching": true,
        "ordering": false,
        "info": false,
    });
    $("#2021").DataTable({
        "responsive": true,
        "lengthChange": true,
        "autoWidth": false,
        "paging": false,
        "searching": true,
        "ordering": false,
        "info": false,
    });
    $("#1920").DataTable({
        "responsive": true,
        "lengthChange": true,
        "autoWidth": false,
        "paging": false,
        "searching": true,
        "ordering": false,
        "info": false,
    });
</script>